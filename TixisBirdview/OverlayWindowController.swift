...
@MainActor
final class OverlayFramePersistenceGate {
    ...
    func endProgrammaticChange() {
        precondition(programmaticChangeDepth > 0, "Unbalanced programmatic frame change")
        programmaticChangeDepth -= 1
    }

    func perform(_ action: () -> Void) {
        beginProgrammaticChange()
        defer { endProgrammaticChange() }
        action()
    }
}
...
final class OverlayWindowController: NSObject, NSWindowDelegate {
    ...
    func show() {
        ...
        framePersistenceGate.beginProgrammaticChange()
    ...
    func windowDidMove(_ notification: Notification) {
        guard framePersistenceGate.allowsPersistence else {
            return
        }
